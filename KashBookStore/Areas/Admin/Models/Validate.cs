 ...
 namespace KashBookStore.Areas.Admin.Models
{
     ...
     *Check whether first name and last name are in database should only happy on insert,
     ...
     *the Author name fields to be changed. So, on edit, GenreID is read-only field, so it's
     *not changed by user, so no check is needed. Thus, Genre check doesn't need additional
     *'Operation' field.
     *
     *Genre check is necessary because if trying to add a GenreID that already exists in database, will
    ...
    public class Validate
    {
        ...
        public void CheckAuthor(string firstName, string lastName, string operation, Repository<Author> data)
        {
            ...
            IsValid = (entity == null) ? true : false;
            ErrorMessage = (IsValid) ? "" : $"The Author {entity.FullName} is already in the database";
        }
