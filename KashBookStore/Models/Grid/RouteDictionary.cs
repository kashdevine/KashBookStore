...
namespace KashBookStore.Models.Grid
{
    public class RouteDictionary : Dictionary<string, string>
    {
        ...
        {
            // author filter contains prefix, author id, and slug(e.g., author-8-ta-nehisi-coates).
            // only need author id for filter, so first remove 'author-' prefix from string. At that
            // point, the authorid will be at the beginnning of the string. so find index of dash after
            // id number, and then return substring from the beginning of the string to that index.
            get
            {
